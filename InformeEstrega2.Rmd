...
```{r modelo1_class}
modelo1_clasificacion <- rpart(precio_categoria~.,data = train, method = "class")
rpart.plot(modelo1_clasificacion)
```

```{r matriz de confusion}
predicciones_clas<-predict(modelo1_clasificacion, newdata = train, type = "class")
matriz_confusion<- table(train$precio_categoria, predicciones_clas)
print(matriz_confusion)
```

Según la matriz de confusión, el modelo tiene una precision general de 81.7%, por lo que clasifica correctamente la mayoria de las viviendas, con dificulades para casificar correctamente las categorias de economicas y de lujo.
