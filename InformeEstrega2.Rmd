...
```{r modelo1_class}
rpart.plot(modelo1_clasificacion, type = 4, extra=1)
```
...
predicciones_clas<-predict(modelo1_clasificacion, newdata = test_set)
...
cfm <- confusionMatrix(predicciones_test, test_set$precio_categoria)
print(cfm)
```

Según la matriz de confusión el modelo clasifica de buena manera las categorias economica y Lujo con precision alta, aunque hay cienta confusion entre las categorias intermedia y cara, donde se clasifica algunas casas en intermedia cuando es cara y viceversa. 

Las categorias muestran alta sencibilidad y y especificidad por lo que el modelo identifica correctamente las viviendas en cada categoría, la categoria Intermedia tiene el valor mas alto de 0.9892 a comparacion con Lujo que tiene 0.8000, por que concluye que el modelo tiende a clasificar de mejor manera la categoria Intermedia que Lujo a comparacion con intermedia.

Notese que como valor de Accuracy hay 88.89%, por lo que el modelo tiene buen rendimiento.
...
predicciones_test<-predict(modelo1_clasificacion, newdata=test_set, type = "class")
